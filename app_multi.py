...
import pickle
#Seven Algorithm
dt_clf_model=pickle.load(open('dt_clf_model.sav','rb'))
knn_clf_model=pickle.load(open('knn_clf_model.sav','rb'))
lr_clf_model=pickle.load(open('lr_clf_model.sav','rb'))
nb_clf_model=pickle.load(open('nb_clf_model.sav','rb'))
rf_clf_model=pickle.load(open('rf_clf_model.sav','rb'))
gb_clf_model=pickle.load(open('gb_clf_model.sav','rb'))
svm_clf_model=pickle.load(open('svm_clf_model.sav','rb'))

#Ensemble Models
bagging_clf_model=pickle.load(open('bagging_clf_model.sav','rb'))
boosting_clf_model=pickle.load(open('boosting_clf_model.sav','rb'))
stacking_clf_model=pickle.load(open('stacking_clf_model.sav','rb'))
voting_clf_model=pickle.load(open('voting_clf_model.sav','rb'))
...
def main():
    ...
    activities=['Decision Tree','K-Nearest Neighbors','Logistic Regression','Multinomial Naive Bayes','Random Forest','Gradient Boosting','Support Vector Machines','Bagging Ensemble','Boosting Ensemble','Stacking Ensemble','Voting Ensemble']
    ...
    if st.button('Predict'):
        if option=='Decision Tree':
            st.success(price(dt_clf_model.predict(inputs)))
        elif option=='K-Nearest Neighbors':
            st.success(price(knn_clf_model.predict(inputs)))
        elif option=='Logistic Regression':
            st.success(price(lr_clf_model.predict(inputs)))
        elif option=='Multinomial Naive Bayes':
            st.success(price(nb_clf_model.predict(inputs)))
        elif option=='Random Forest':
            st.success(price(rf_clf_model.predict(inputs)))
        elif option=='Gradient Boosting':
            st.success(price(gb_clf_model.predict(inputs)))
        elif option=='Support Vector Machines':
            st.success(price(svm_clf_model.predict(inputs)))
        elif option=='Bagging Ensemble':
            st.success(price(bagging_clf_model.predict(inputs)))
        elif option=='Boosting Ensemble':
            st.success(price(boosting_clf_model.predict(inputs)))
        elif option=='Stacking Ensemble':
            st.success(price(stacking_clf_model.predict(inputs)))
        else:
           st.success(price(voting_clf_model.predict(inputs)))
